...
st.write("✅ Loaded People:", list(set(classNames)))
...
import streamlit as st   # make sure this is at top
    ...
    if st.button("🔍 Detect Face"):
        ...
        if len(encodesCurFrame) == 0:
            ...
            for encodeFace, faceLoc in zip(encodesCurFrame, facesCurFrame):
                ...
                if len(faceDis) == 0:
                    st.error("❌ No known faces available")
                    continue

                matchIndex = np.argmin(faceDis)

                st.write(f"🔍 Distance Score: {faceDis[matchIndex]:.4f}")
